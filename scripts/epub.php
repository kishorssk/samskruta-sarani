...
class epub {
	...
	public function createEPUBFile($id,$isbn='') {
		...
		$files = glob($id . '/OEBPS/*.xhtml');

		foreach($files as $file) {

			if(preg_match('/.*-print.xhtml/', $file)){
	...
	public function getNumberedXhtmlFiles($id) {
		...
		foreach ($files as $file) {
			...
			if(preg_match('/^[0-9]+[a-z]*\-.*\.xhtml/', str_replace(UNICODE_SRC . $id . '/', '', $file)) && !preg_match('/.*?-print\.xhtml/', str_replace(UNICODE_SRC . $id . '/', '', $file)))
				array_push($numberedFiles, $file);
		}
	...
	public function printContentOpf($id, $sectionHierarchy, $bookDetails, $numberedFiles, $allFiles) {
		...
		$template['dateModified'] = date('Y-m-d') . 'T' . date('H:i:s') . 'Z';
