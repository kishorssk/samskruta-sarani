...
class Stages{
	...
	public function parseHTML($html) {
		...
		foreach($xpath->query('//text()') as $text_node) {
			...
			$text_node->nodeValue = $this->shreelipi2Unicode($text_node->nodeValue);
		}
	...
	public function processRawHTML($text) {
		...
		$text = preg_replace('/<SPAN .*font.*(times|Sylfaen|Tahoma|Arial).*>/i', '<SPAN-class="en">', $text);
	...
	public function shreelipi2Unicode ($text) {
		...
		//~ $text = str_replace('Ÿ', '·', $text); //manu

		// ya group
		$text = str_replace('¿åé', 'ಯ್​', $text); // Caution! zero width space found after halanta
		...
		// ma group
		$text = str_replace('ÊÜåé', 'ಮ್​', $text); // Caution! zero width space found after halanta
		...
		$text = str_replace('-', '', $text); // soft hyphen ?
		...
		$text = str_replace('é', '್​', $text); // Caution! zero width space found after halanta
		...
		//~ $text = str_replace('ñ', 'ತ್', $text);//manu
