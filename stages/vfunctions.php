<?php
	...
	function getAllFiles($bookID) {
	    ...
	    foreach($iterator as $file => $object) {
	    	if(preg_match('/.*(html|ven)$/i',$file)) array_push($allFiles, $file);
	    }
	...
	function process($bookID, $file) {
		...
		foreach ($content as $line) {
			...
			elseif(preg_match('/<html>/i', $line)) $insideBody = true;
	...
	function replaceTags($line) {
		...
		$line = preg_replace('/<sup>(.*?)<\/sup>/i', '<sup><a epub:type="noteref" href="999-aside.xhtml#id-">' . "$1" . '</a></sup>', $line);
