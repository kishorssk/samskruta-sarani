...
class Stages{
	...
	public function formHTML ($text) {
		...
		$html = preg_replace("/([0-9])&lt;P255DJ0&gt;/", "<sup>$1</sup>", $html);
		// English footnotes
		...
		//~ $html = preg_replace("/([0-9])®MDNM/", '<sup><a epub:type="noteref" href="999-aside.xhtml#id-1.3">' . "$1" . '</a></sup>', $html);
		
		
		$html = preg_replace("/([0-9])®MDNM/", "<sup>$1</sup>", $html);
		$html = preg_replace("/&lt;[A-Z\.0-9]*B&gt;(.*?)&lt;[A-Z\.0-9]*D&gt;/", "<strong>$1</strong>", $html);
		$html = preg_replace("/&lt;F[0-9]{2}.*?&gt;(.*?)&lt;F[0-9]{3}.*?&gt;/", '<span class="en">' . "$1" . '</span>', $html);
